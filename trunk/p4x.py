...
class PList(PBase):
    ...
    for pnode in self.__dict__['@nodes']:
      ...
      for key, value in kwargs.iteritems():
        ...
        if not attrs.has_key(key):
          ...
          if attrs[key].value != unicode(value):
            match = False
            break
      if match:
        nodes.append(pnode)
  ...
  def __nonzero__(self):
    return len(self.__dict__['@nodes'])
...
class PNode(PBase):
  ...
  def __getattr__(self, key):
    ...
    if key.startswith('_'):
      # attribute
      attr = key[1:]
      if self.__dict__['@dom'].attributes.has_key(attr):
